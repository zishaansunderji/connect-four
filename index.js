...
class Board {
    ...
    async add(y, x, turn) {
        for (let i = 0; i <= y; i++) {
            const speed = (1000 / y) ** 0.5;
            if (i > 0 && y !== 0) this.board[i - 1][x] = '-';
            this.board[i][x] = turn;
            await sleep(speed);
        }
        isAnimating = false;
...
async function addToBoard(isEmpty, x, TURN) {
    await board.add(isEmpty, x, TURN);
    undo.push([isEmpty, x]);
...
async function mousePressed() {
    if (!isAnimating) {
        ...
        if (isEmpty !== -1 && !isWin) {
            isAnimating = true;
            await addToBoard(isEmpty, x, TURN);
        }
    }
}
    ...
    if (!isAnimating) {
        if (keyIsDown(CONTROL) && key == 'z') {
            ...
            if (redo.length > 0) {
                ...
                await board.add(coords[0], coords[1], TURN);
